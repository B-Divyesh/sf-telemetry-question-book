...
- Deployment recheck: the managed deployment command was run from `c790e79`. A new cold two-viewport route sweep passed `/`, `/demo`, `/book`, `/privacy`, `/terms`, `/snapshot`, the Northstar source page, and the designed HTTP 404. The phone `?demo=1` flow showed all four first-card details, created a `d_` expiring link, and returned to `/book` with **Start for real**. No serious or critical axe issue, overflow, app console error, or page error remained.
